...
In order to use the integrated search proxy, you have to add the following
two lines in your application's root file. Usually, this should be the
`__init__.py` containing the `main` function of your application. Insert
`import gevent.monkey` as the first import and call `gevent.monkey.patch_all()`
...
```console
.venv/bin/pip install oereb_client==2.0.0
```
...
> **NOTE:** _pyramid_oereb_ has to be run in a separate container. You have to use the configuration
